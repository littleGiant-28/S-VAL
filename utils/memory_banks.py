...
class BaseMemory(torch.nn.Module):
    ...
    def update(self, tensor, indices):
        self.check_for_double_update(indices)
        self.index_update.extend(indices)
        ...
        self.memory.index_copy_(dim=0, index=indices, tensor=new_memory)
        ...
        with torch.no_grad():
            ...
            for batch in tepoch:
                ...
                self.memory.index_copy_(
                    dim=0, index=indices, tensor=slpd_features
                )
    ...
    def initialize_memory(self, model, dataloader):
        ...
        with torch.no_grad():
            ...
            for batch in tepoch:
                ...
                slpd_features = model.get_td_features(image_patch)
                self.memory.index_copy_(
                    dim=0, index=indices, tensor=slpd_features
                )
